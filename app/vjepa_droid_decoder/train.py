...
from app.vjepa_droid_decoder.utils import init_opt, init_video_model, load_checkpoint, load_pretrained
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def train_step():
                ...
                #
                #
                # def forward_decoder(h):
                #     # Input h: [B, num_tokens, token_dim]
                #     # Convert tokens back to [B, C, T, H, W]
                #     if isinstance(h, list):
                #         h = torch.stack(h, dim=0)
                #
                #     _, num_tokens, token_dim = h.shape
                #
                #     spatial_tokens = tokens_per_frame
                #     assert (
                #         num_tokens % spatial_tokens == 0
                #     ), "Token count must be divisible by spatial tokens per frame"
                #     temporal_tokens = num_tokens // spatial_tokens
                #
                #     grid = crop_size // patch_size
                #     patch_height = patch_size
                #     patch_width = patch_size
                #     frame_patch_size = tubelet_size
                #     channels = clips.size(1)
                #
                #     expected_token_dim = patch_height * patch_width * frame_patch_size * channels
                #     if token_dim != expected_token_dim:
                #         raise ValueError(
                #             f"Decoder token dim {token_dim} does not match expected {expected_token_dim}"
                #         )
                #
                #     reconstructed_clip = rearrange(
                #         h,
                #         "b (f gh gw) (ph pw pf c) -> b c (f pf) (gh ph) (gw pw)",
                #         f=temporal_tokens,
                #         gh=grid,
                #         gw=grid,
                #         ph=patch_height,
