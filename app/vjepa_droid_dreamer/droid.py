...
class DROIDVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # Assuming the CSV contains absolute paths from another machine, we take the basename
        # and prepend the directory of the CSV file.
        base_dir = os.path.dirname(data_path)
        self.samples = [os.path.join(base_dir, os.path.basename(p)) for p in df.iloc[:, 0]]
    ...
    def poses_to_diffs(self, poses):
        ...
        rotvecs = poses[:, 3:6]  # rotation vectors, shape [T, 3]
        matrices = [Rotation.from_rotvec(rv).as_matrix() for rv in rotvecs]
    ...
    def transform_frame(self, poses, extrinsics):
        ...
        def pose_to_transform(pose):
            ...
            rotvec = pose[3:6]  # rotation vector, shape [3]
    ...
    def loadvideo_decord(self, path):
        ...
        # Fix: gripper_position is already (T, 1), so [:, None] makes it (T, 1, 1) which causes mismatch with (T, 6)
        gripper_pos = np.array(trajectory["observation"]["robot_state"]["gripper_position"])
        ...
        if vlen == nframes:
            ...
            ef = np.random.randint(nframes, vlen + 1)
        sf = ef - nframes
