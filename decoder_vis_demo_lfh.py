...
CHECKPOINT_PATH = Path("/nvmessd/yinzi/vjepa2/checkpoints/decoder_train_vitg/e100.pt")
...
def _infer_encoder_config(state_dict):
    ...
    if pos_embed is not None:
        ...
        # 如果没有 pos_embed，尝试从其他层推断
        # 方法1: 从 norm1.weight 获取
        norm1_weight = state_dict.get("blocks.0.norm1.weight")
        if norm1_weight is not None:
            embed_dim = norm1_weight.shape[0]
        else:
            # 方法2: 从 patch_embed 获取
            patch_embed_weight = state_dict.get("patch_embed.proj.weight")
            if patch_embed_weight is not None:
                # patch_embed.proj.weight shape: [embed_dim, 3, tubelet_size, patch_size, patch_size]
                embed_dim = patch_embed_weight.shape[0]
            else:
                # 方法3: 从 qkv weight 推断
                qkv_weight = state_dict.get("blocks.0.attn.qkv.weight")
                if qkv_weight is not None:
                    # qkv weight shape: [embed_dim * 3, embed_dim]
                    embed_dim = qkv_weight.shape[1]
                else:
                    raise ValueError(
                        "Cannot infer encoder embed_dim from state dict. "
                        "Missing pos_embed, norm1.weight, patch_embed.proj.weight, and qkv.weight"
                    )
...
video_path = Path('/nvmessd/yinzi/vjepa2/lfh_experiment/success.mp4')
...
if _MATPLOTLIB_AVAILABLE:
    ...
    output_path = Path("decoder_vis_demo_comparison1.png").resolve()
